...
public abstract class Neo4jProcessor {
	...
	protected final HashObjLongMap<String>	tempResourcesIndex;
	protected final HashObjLongMap<String>	tempResourcesWDataModelIndex;
	protected final HashObjLongMap<String>	tempResourceTypes;

	private BatchInserterIndex				values;
	protected final HashObjLongMap<String>	bnodes;
	// private BatchInserterIndex statementHashes;
	private ChronicleMap<Long, Long>		statementHashes;

	protected final HashLongLongMap			tempStatementHashes;
	...
	public Neo4jProcessor(final BatchInserter inserter) throws DMPGraphException {
		...
		bnodes = HashObjLongMaps.getDefaultFactory().newMutableMap();
		nodeResourceMap = HashLongObjMaps.getDefaultFactory().newMutableMap();

		tempResourcesIndex = HashObjLongMaps.getDefaultFactory().newMutableMap();
		tempResourcesWDataModelIndex = HashObjLongMaps.getDefaultFactory().newMutableMap();
		tempResourceTypes = HashObjLongMaps.getDefaultFactory().newMutableMap();
		// tempStatementHashes = new LongLongOpenHashMap();
		// tempStatementHashes = ChronicleMapUtils.createOrGetLongIndex();
		tempStatementHashes = HashLongLongMaps.getDefaultFactory().newMutableMap();
	...
	private void copyNFlushNClearIndex(final HashObjLongMap<String> tempIndex, final BatchInserterIndex neo4jIndex, final String indexProperty,
		...
		final HashObjSet<Map.Entry<String, Long>> entrySet = tempIndex.entrySet();
		final ObjCursor<Map.Entry<String, Long>> iter = entrySet.cursor();

		while (iter.moveNext()) {

			final Map.Entry<String, Long> entry = iter.elem();
			...
			neo4jIndex.add(entry.getValue(), MapUtil.map(indexProperty, entry.getKey().toCharArray()));
	...
	private void copyNFlushNClearLongIndex(final HashLongLongMap tempIndex, final ChronicleMap<Long, Long> persistentIndex, final String indexName) {
		...
		// for (int i = 0; i < states.length; i++) {
		final HashObjSet<Map.Entry<Long, Long>> entrySet = tempIndex.entrySet();
		final ObjCursor<Map.Entry<Long, Long>> iter = entrySet.cursor();

		while (iter.moveNext()) {

			final Map.Entry<Long, Long> entry = iter.elem();

			// if (states[i]) {

			// persistentIndex.add(values[i], MapUtil.map(indexProperty, keys[i]));
			persistentIndex.acquireUsing(entry.getKey(), entry.getValue());
	...
	private Optional<Long> getIdFromIndex(final String key, final HashObjLongMap<String> tempIndex, final BatchInserterIndex index,
